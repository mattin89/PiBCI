...
class GraphWindow(tk.Toplevel):
    def __init__(self, main_window):
        ...
        self.data = np.random.uniform(0, 0, (8, 160))
    ...
    def show_image(self):
        ...
        if self.total_repetitions > 0:
            ...
            if self.c_left >= self.total_repetitions_unch / 2:
                ...
                image = random.choice(["left2.png", "right2.png"])
                
            self.label = "left" if image == "left2.png" else "right"    
            ...
            #self.session_data.append([self.How can_data() for _ in range(8)] + [label])

            self.total_repetitions -= 1
            
            self.show_nothing()
            #self.after(self.get_data, self.show_nothing)
        else:
    ...
    def get_data(self):
        ...
        while time.time() - start_time < self.display_time:
                ...
                if len(data) == self.num_pins * 4:
                        for i in range(0, len(data), 4):
                                byte_chunk = data[i:i+4]
                                value = int.from_bytes(byte_chunk, byteorder='little', signed=False)
                                new_data[i // 4].append(value)
                        print("Inside")
                        print(new_data)
        
        self.session_data.append(new_data + [self.label])
        self.sock.close()
        ...
        if save_file:
            ...
            with open(save_file, "w", newline="") as csvfile:
                ...
                for sublist in self.session_data:
                    row = sublist[:-1]
                    string_value = sublist[-1]
                    
                    writer.writerow(row + [string_value])
...
class ArrowWindow(tk.Toplevel):
    def __init__(self, main_window):
        ...
        self.left_image = ImageTk.PhotoImage(Image.open("left2.png"))
        self.right_image = ImageTk.PhotoImage(Image.open("right2.png"))
